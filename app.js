...
/* ---------- live real-world date & time ---------- */

// This is the viewer's own clock - unrelated to the simulated/live network
// clock chip beside it, which tracks the city being planned for. It ticks
// once a minute, on the minute, so it never drifts and never redraws for no
// visible change.
function tickLiveClock() {
  ...
  const date = new Intl.DateTimeFormat(undefined,
    { weekday: 'short', month: 'short', day: 'numeric' }).format(now);
  const time = new Intl.DateTimeFormat(undefined,
    { hour: 'numeric', minute: '2-digit' }).format(now);
...
function startLiveClock() {
  ...
  setTimeout(() => { tickLiveClock(); setInterval(tickLiveClock, 60000); }, msToNextMinute);
}
